...
void * linked_list_starting_address;
void * final_pointer;
int linked_size;
void * pointers[10];
int number;
void mems_init(){
    number=0;
    linked_list_starting_address=mmap(NULL,PAGE_SIZE,PROT_READ | PROT_WRITE | PROT_EXEC , MAP_PRIVATE|MAP_ANONYMOUS ,0,0);
    head=(struct main*)linked_list_starting_address;
    pointers[number]=linked_list_starting_address;
    number++;
    final_pointer=linked_list_starting_address+sizeof(struct main);
    ...
    linked_size=PAGE_SIZE;
    Mems_Starting_Virtual_Address=1000;
    head->mems_virtual_address=Mems_Starting_Virtual_Address;      
}

void * increase_size(){
    if((unsigned long)final_pointer-(unsigned long)linked_list_starting_address>linked_size){
        final_pointer=mmap(NULL,PAGE_SIZE,PROT_READ | PROT_WRITE | PROT_EXEC , MAP_PRIVATE|MAP_ANONYMOUS ,0,0);
        linked_size+=PAGE_SIZE;
        pointers[number]=final_pointer;
        number++;
    }
...
void mems_finish(){
    ...
    while(remove_main!=NULL){
        ...
        remove_main=remove_main->next;
    }   
    for(int i;i<number;i++){
        munmap(pointers[number],4096);
    }
}
...
void* mems_malloc(size_t size){
    ...
    while(iterator!=NULL){
        ...
        while(sub_iterator!=NULL){
            ...
            if(sub_iterator->size>=size && sub_iterator->type=="HOLE"){
                ...
                struct sub *new_sub_node=(struct sub*)increase_size();
                final_pointer+=sizeof(struct sub);
    ...
    if(iterator==NULL){
        ...
        struct main *new_main_node=(struct main*)increase_size();
        final_pointer+=sizeof(struct main);
        ...
        struct sub *new_sub_node_1=(struct sub*)increase_size();
        final_pointer+=sizeof(struct sub);

        
        struct sub *new_sub_node_2=(struct sub*)increase_size();
        final_pointer+=sizeof(struct sub);
